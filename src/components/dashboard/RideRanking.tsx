  ...
  return (
    <div className="space-y-2.5">
      {[0, 1, 2, 3, 4].map((i) => (
        <div
          key={i}
          className="flex items-center gap-3 p-3 rounded-xl bg-navy-900/40 border border-navy-700/30"
        >
          <div className="w-7 h-7 rounded-lg bg-metal-700/40 animate-pulse" />
          <div className="w-32 space-y-2">
            <div className="h-4 w-20 bg-metal-700/40 rounded animate-pulse" />
            <div className="h-3 w-16 bg-metal-700/30 rounded animate-pulse" />
          </div>
          <div className="flex-1 h-2 bg-metal-700/30 rounded-full animate-pulse" />
          <div className="w-20 h-4 bg-metal-700/40 rounded animate-pulse" />
        </div>
      ))}
    </div>
  );
...
export default function RideRanking() {
  ...
  const isLoading = rawRides.length === 0;
