  ...
  return (
    <div className="space-y-2.5 max-h-72 overflow-y-auto pr-1">
      {[0, 1, 2].map((i) => (
        <div
          key={i}
          className="flex items-start gap-3 p-3.5 rounded-xl bg-navy-900/40 border border-navy-700/30"
        >
          <div className="w-2.5 h-2.5 rounded-full bg-metal-700/50 animate-pulse mt-1" />
          <div className="flex-1 space-y-2">
            <div className="h-4 w-40 bg-metal-700/40 rounded animate-pulse" />
            <div className="h-3 w-full bg-metal-700/30 rounded animate-pulse" />
            <div className="h-3 w-24 bg-metal-700/20 rounded animate-pulse" />
          </div>
        </div>
      ))}
    </div>
...
export default function RealtimeAlertList() {
  ...
  const isLoading = rawAlerts.length === 0;
